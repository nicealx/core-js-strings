...
function countVowels(str) {
  return str
    .split('')
    .reduce(
      (sum, s) => ('aeiouy'.includes(s.toLowerCase()) ? sum + 1 : sum + 0),
      0
    );
}
...
function isPalindrome(str) {
  ...
  return (
    ...
    str.toLowerCase().replaceAll(regexp, '').split('').reverse().join('')
  );
}
...
function invertCase(str) {
  return str
    .split('')
    .map((e) => (e.match(/[A-Z]/) ? e.toLowerCase() : e.toUpperCase()))
    .join('');
}
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((word) =>
      word
        .split('')
        .map((s) => (normal.indexOf(s) !== -1 ? rot[normal.indexOf(s)] : s))
        .join('')
    )
    .join(' ');
}
